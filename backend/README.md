...
AI Service может добавить findings и evidence без изменения backend. HTTP ошибка, некорректный JSON или таймаут 600 секунд переводит анализ в `FAILED`; `/run` возвращает 502. Повторный `/run` разрешён после ошибки. Новый документ сбрасывает прежний результат.
